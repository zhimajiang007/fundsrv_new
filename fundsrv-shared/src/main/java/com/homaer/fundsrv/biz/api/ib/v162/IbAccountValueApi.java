package com.homaer.fundsrv.biz.api.ib.v162;
 ...
 */
@Api(value = "/ib/getAccountValues", version = { "1.6.2" }, osType = { OsType.IOS, OsType.ANDROID })
public class IbAccountValueApi extends AbstractApi<IbResponse, IbRequest>
{
  ...
  public IbResponse execute(IbRequest request)
  {
    ...
    try
    {
      ...
      // if (null != cacheManager.get(request.getAccount() + "-Positions")) {
      // System.out.println("直接缓存取值1.6.2");
      // lstPositions = (List<PositionsInfo>)
      // cacheManager.get(request.getAccount() + "-Positions");
      // }
      ...
      else
      {
        ...
        for (int i = 0; i < lstPositions.size(); i++)
        {
          ...
          // 日志方便查看
          System.out.println("第" + (i + 1) + "股票版本1.6.2");
      ...
      if (null != objMarket)
      {
        ...
        else
        {
          ...
          // 初始资产
          BigDecimal accountInitial = netLiquidationByCurrency.subtract(unrealizedPnl);
          // 当前涨跌百分比
          BigDecimal accountRiseFall = unrealizedPnl.divide(accountInitial, 6, RoundingMode.DOWN);
        ...
        System.out.println(objMarket.getStockMarketValue() + "股票市值1.6.2");
