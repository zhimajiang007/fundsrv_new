 ...
 * 
 */
@Api(value = "/ib/reqPositions", version = { "1.6.1" }, osType = { OsType.ALL})
			...
			} else
			{
				for (int i = 0; i < lstPositions.size(); i++)
				{
					...
					if (objPosition.getAvgCost() <= 0 || objPosition.getPosition() <= 0) {
						...
						System.out.println("删除一条无效持仓222" + objPosition.getSymbol());
						continue;
					}					
				}
				...
				for (int i = 0; i < lstPositions.size(); i++)
				{
					...
					// 股票涨跌百分比
					System.out.println("objPosition.getCurrentPrice()" + objPosition.getCurrentPrice());
					System.out.println("objPosition.getAvgCost()" + objPosition.getAvgCost());
					String riseFall = String.format("%.6f", (objPosition.getCurrentPrice() - objPosition.getAvgCost())/objPosition.getAvgCost()) + "";
					System.out.println(riseFall);
...
//					String rates = nFromat.format(d);
					...
					System.out.println(lstPositions.get(i).getRiseFall() + "版本--1.6.1");
				}
			}
