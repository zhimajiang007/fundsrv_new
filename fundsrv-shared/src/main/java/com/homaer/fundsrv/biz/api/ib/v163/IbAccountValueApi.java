 ...
 * 2016年3月10日
 * 
 * 比1.6.2修改"股票账户总盈亏百分比"为"股票市值盈亏百分比"，修改算法
 * 
 * @author changhai.wang
 */
@Api(value = "/ib/getAccountValues", version = { "1.6.3" }, osType = { OsType.IOS, OsType.ANDROID })
public class IbAccountValueApi extends AbstractApi<IbResponse, IbRequest>
{
  ...
  public IbResponse execute(IbRequest request)
  {
    ...
    try
    {
      ...
      // oldPositions2只在IB推送end时更新
      if (null != cacheManager.get(request.getAccount() + "-newPositions"))
      {
        System.out.println("直接缓存取值1.6.3");
        lstPositions = (List<PositionsInfo>) cacheManager.get(request.getAccount() + "-newPositions");
        if (CollectionUtils.isEmpty(lstPositions))
        {
          System.out.println("newPositions取值持仓为空");
          lstPositions = (List<PositionsInfo>) cacheManager.get(request.getAccount() + "-oldPositions");
        }
        // if (CollectionUtils.isEmpty(lstPositions)){
        // System.out.println("old取值持仓为空");
        // lstPositions = (List<PositionsInfo>)
        // cacheManager.get(request.getAccount() + "-newPositions");
        // }
      }
      Date updateDate = (Date) cacheManager.get(request.getAccount() + "-PositionsTime");
      if (null != updateDate)
      {
        Date now = new Date();
        long changeTime = (now.getTime() - updateDate.getTime()) / 60000;
        System.out.println(changeTime + "时间对比");
        if (changeTime >= 5)
        {
          needFresh = "1";
        }
      ...
      else
      {
        for (int i = 0; i < lstPositions.size(); i++)
        {
          ...
          if (objPosition.getAvgCost() <= 0 || objPosition.getPosition() <= 0)
          {
            lstPositions.remove(i);
            System.out.println("删除一条无效持仓getAccountValues" + objPosition.getSymbol());
            continue;
          }
        }
        for (int i = 0; i < lstPositions.size(); i++)
        {
          PositionsInfo objPosition = lstPositions.get(i);
          if ("USD".equalsIgnoreCase(objPosition.getSymbol()))
          {
            System.out.println("getAccountValues过滤掉USD");
            continue;
          }
          // yahoo查询当前股价及股票名称
          ...
          // 日志方便查看
          System.out.println("第" + (i + 1) + "股票版本1.6.3");
      ...
      if (null != objMarket)
      {
        ...
        System.out.println("objMarket.getCashBalance()" + objMarket.getCashBalance());
        ...
        {
          // 初始资产
          BigDecimal accountInitial = stockMarketValue.subtract(unrealizedPnl);
          // 当前涨跌百分比
          BigDecimal accountRiseFall = new BigDecimal(0);
          if (accountInitial.compareTo(new BigDecimal(0)) > 0)
          {
            accountRiseFall = unrealizedPnl.divide(accountInitial, 6, RoundingMode.DOWN);
          }
          System.out.println(accountRiseFall);
          NumberFormat percent = NumberFormat.getPercentInstance();
          percent.setMaximumFractionDigits(2);
          System.out.println(percent.format(accountRiseFall.setScale(4, BigDecimal.ROUND_DOWN).doubleValue()));
          objMarket.setAccountRiseFallCost(unrealizedPnl + "");
          objMarket.setAccountRiseFall(percent.format(accountRiseFall.setScale(4, BigDecimal.ROUND_DOWN).doubleValue()));
          objMarket.setAccountInitial("" + unrealizedPnl.add(stockMarketValue.add(new BigDecimal(objMarket.getCashBalance()))));
        }
        System.out.println(objMarket.getStockMarketValue() + "股票市值1.6.3");
        ...
        System.out.println(objMarket.getAccountRiseFall() + "股票市值盈亏百分比");
