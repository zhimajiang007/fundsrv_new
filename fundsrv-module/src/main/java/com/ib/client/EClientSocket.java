...
public class EClientSocket {
    ...
    public EClientSocket( AnyWrapper anyWrapper) throws SocketException {
        m_anyWrapper = anyWrapper;
		m_clientId = InitContants.TWS_CLIENTID;
        m_extraAuth = false;
        m_connected = false;
        m_serverVersion = 0;
    }
    
    public synchronized void eConnect( String host, int port, int clientId) throws SocketException {
    ...
    public synchronized void eConnect( String host, int port, int clientId, boolean extraAuth) {
        ...
        System.out.println("m_clientId===" + m_clientId);
        m_extraAuth = extraAuth;
    ...
    public synchronized void eConnect(Socket socket, int clientId) throws IOException {
        m_clientId = clientId;
        System.out.println("====111===" + m_clientId);
        eConnect(socket);
    }
    ...
    public synchronized void reqExecutions(int reqId, ExecutionFilter filter) {
    	System.out.println("发送请求222---" + reqId + "====" + filter.m_clientId + "," + filter.m_time);
        // not connected?
        if( !m_connected) {
            notConnected();
            return;
    ...
    public synchronized void reqPositions() {
    	System.out.println("发送持仓请求====" + new Date());
        // not connected?
        if( !m_connected) {
            notConnected();
            return;
    ...
    public synchronized void cancelPositions() {
    	System.out.println("cancelPositions请求");
        // not connected?
        if( !m_connected) {
            notConnected();
            return;
